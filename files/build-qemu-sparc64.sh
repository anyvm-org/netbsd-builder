...
# permanently wedging both devices' interrupts. NetBSD sparc64 hits it as
# the "cmdide0: lost interrupt" / "wm0: device timeout" storm under
# concurrent disk+NIC DMA, and it is why 11.0 sparc64 boots randomly hung
# and shutdowns crawled. files/qemu-sabre-irq-clobber.patch fixes it (one
# condition, mirroring the OBIO branch); with it, the 11.0 single-disk image
# survives repeated concurrent scp+dd stress with zero lost interrupts and
# shuts down in seconds. (The 10.x hybrid-disk images sidestep the bug even
# on stock QEMU; the pin makes them uniformly solid too.)
#
# Firmware notes:
#  - openbios-sparc64: the sun4u machine firmware.
#  - efi-e1000.rom: option ROM for the e1000 NIC every conf uses (VM_NIC=
#    e1000); QEMU 10.x ABORTS at launch without it ("failed to find romfile
#    efi-e1000.rom"), so it must ride along in the pruned tree.
#
...
# it beside the image assets; anyvm.py downloads it at run time on Linux
# x86_64 hosts (PINNED_QEMU_ASSETS/PINNED_QEMU_REPOS) and falls back to the
# system QEMU elsewhere. Self-contained by design: builders never reference
# other builders' files or release assets.
set -e
